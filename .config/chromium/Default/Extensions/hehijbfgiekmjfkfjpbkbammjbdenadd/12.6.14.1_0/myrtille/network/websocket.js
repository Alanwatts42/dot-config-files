...
function Websocket(config, dialog, display, network)
{
    ...
    this.init = function ()
    {
        ...
        if (config.getImageMode() == config.getImageModeEnum().BINARY)
        {
            ...
            {
                dialog.showDebug('websocket binary init error: ' + exc.message + ', falling back to base64 (or roundtrip if not available)');
                config.setImageMode(!true ? config.getImageModeEnum().ROUNDTRIP : config.getImageModeEnum().BASE64);
                dialog.showStat(dialog.getShowStatEnum().IMAGE_MODE, config.getImageMode());
            }
        }
    ...
    function close(e)
    {
        ...
        else
        {
            ...
            dialog.showDebug('websocket connection closed with error code ' + e.code + ' (if you have IIS 8 or greater, ensure the websocket protocol is enabled)');
        }
        if (network.onclosed) {
            network.onclosed();
        }
            ...
            if (data != null && data != '')
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        width = imgInfo[3];
                        height = imgInfo[4];
                        format = config.display.getFormatText(imgInfo[5]);
                        quality = imgInfo[6];
